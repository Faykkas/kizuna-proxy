...
export default function TokyoInStoreClient({ locale } = {}) {
  const { lang, t } = useLang();
  const announce = useAnnounce();
  useScrollReveal();

  const data = landingTranslations.tokyoInStore;
        ...
        <div className="blog-page">
          <div className="blog-wrap">
            ...
            <p style={{ marginTop: "2rem", fontSize: ".82rem" }}>
              {g.linksLabel} <a href="/services">{g.linkServices}</a> · <a href="/pricing">{g.linkPricing}</a> · <a href="/how-it-works">{g.linkHiw}</a> · <a href="/reviews">{g.linkReviews}</a>
            </p>
          </div>
        </div>
      </main>
