...
//
// `t` is the `form` block of businessSourcingTranslations[locale] — every
// label/placeholder/option falls back to English if a key is missing.
...
export default function BusinessRequestForm({ t = {} }) {
  ...
  const businessTypes = t.businessTypeOptions?.length ? t.businessTypeOptions : DEFAULT_BUSINESS_TYPES;
  const discoveryOptions = t.discoveryOptions?.length ? t.discoveryOptions : DEFAULT_DISCOVERY_OPTIONS;
  ...
  function validate() {
    const e = {};
    if (!form.contactName.trim()) e.contactName = t.errContactName || "Contact name is required.";
    if (!form.businessName.trim()) e.businessName = t.errBusinessName || "Business name is required.";
    if (!form.email.trim()) e.email = t.errEmail || "We need an email to reply.";
    else if (!form.email.includes("@")) e.email = t.errEmailInvalid || "That email looks wrong.";
    if (!form.country.trim()) e.country = t.errCountry || "Where are you shipping to?";
    if (!form.productCategory.trim() && !form.itemLinks.trim() && !form.additionalDetails.trim()) {
      e.productCategory = t.errProductCategory || "Tell us what you're sourcing — a category, a link, or a few details.";
    }
    setErrors(e);
  ...
  if (status === "success") {
    return (
      <div className="req-form req-success">
        <p className="req-success-title">{t.successTitle || "GOT IT"}</p>
        <p className="req-success-desc">
          {t.successDesc || "Thank you. We will review your sourcing request and contact you with the next steps."}
        </p>
    ...
    <div className="req-form">
      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-contact-name">{t.contactName || "Contact name"}</label>
        ...
        <div className="f-field">
          <label htmlFor="bs-business-name">{t.businessName || "Business name"}</label>
      ...
      <div className="f-field">
        <label htmlFor="bs-website">{t.businessWebsite || "Business website or marketplace profile"}</label>
      ...
      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-email">{t.businessEmail || "Business email"}</label>
        ...
        <div className="f-field">
          <label htmlFor="bs-country">{t.country || "Country"}</label>
          <input id="bs-country" type="text" placeholder={t.countryPlaceholder || "France, USA, Australia…"} value={form.country} onChange={e => update("country", e.target.value)} autoComplete="country-name" />
      ...
      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-type">{t.businessType || "Type of business"}</label>
          <select id="bs-type" value={form.businessType} onChange={e => update("businessType", e.target.value)}>
            ...
            {businessTypes.map(v => <option key={v} value={v}>{v}</option>)}
          </select>
        </div>
        <div className="f-field">
          <label htmlFor="bs-category">{t.productCategory || "Product category"}</label>
          <input id="bs-category" type="text" placeholder={t.productCategoryPlaceholder || "Pokémon cards, streetwear, figures…"} value={form.productCategory} onChange={e => update("productCategory", e.target.value)} />
          {errors.productCategory && <span className="f-err">{errors.productCategory}</span>}
        </div>
      </div>

      <div className="f-field">
        <label htmlFor="bs-links">{t.links || "Official event, store or product links"}</label>
        <input id="bs-links" type="text" placeholder="https://…" value={form.itemLinks} onChange={e => update("itemLinks", e.target.value)} />
      </div>

      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-quantity">{t.quantity || "Desired quantities"}</label>
          <input id="bs-quantity" type="text" placeholder={t.quantityPlaceholder || "e.g. 50 units"} value={form.quantity} onChange={e => update("quantity", e.target.value)} />
        </div>
        <div className="f-field">
          <label htmlFor="bs-budget">{t.budget || "Merchandise budget"}</label>
          <input id="bs-budget" type="text" placeholder={t.budgetPlaceholder || "¥50,000 · $500 · flexible"} value={form.budget} onChange={e => update("budget", e.target.value)} />
        </div>
      </div>

      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-deadline">{t.deadline || "Target date or deadline"}</label>
          <input id="bs-deadline" type="date" value={form.deadline} onChange={e => update("deadline", e.target.value)} />
        </div>
        <div className="f-field">
          <label htmlFor="bs-alternatives">{t.alternatives || "Accepted alternatives"}</label>
          <input id="bs-alternatives" type="text" placeholder={t.alternativesPlaceholder || "Substitute colors, sizes, similar items…"} value={form.alternatives} onChange={e => update("alternatives", e.target.value)} />
        </div>
      </div>

      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-partial">{t.partial || "Partial fulfilment accepted"}</label>
        ...
        <div className="f-field">
          <label htmlFor="bs-recurring">{t.recurring || "Recurring sourcing required"}</label>
          <select id="bs-recurring" value={form.recurringSourcing} onChange={e => update("recurringSourcing", e.target.value)}>
            <option value="">{t.selectPlaceholder || "Select…"}</option>
            <option value="yes">{t.yes || "Yes"}</option>
            <option value="no">{t.no || "No"}</option>
            <option value="not_yet">{t.notYet || "Not yet"}</option>
          </select>
        </div>
      </div>
      ...
      <div className="f-row">
        <div className="f-field">
          <label htmlFor="bs-platform">{t.platform || "Preferred live communication platform"}</label>
          <input id="bs-platform" type="text" placeholder={t.platformPlaceholder || "Email, WhatsApp, Discord…"} value={form.contactPlatform} onChange={e => update("contactPlatform", e.target.value)} />
        </div>
        <div className="f-field">
          <label htmlFor="bs-discovery">{t.discovery || "How did you discover Kizuna Proxy?"}</label>
          <select id="bs-discovery" value={form.discoverySource} onChange={e => update("discoverySource", e.target.value)}>
            <option value="">{t.selectPlaceholder || "Select…"}</option>
            {discoveryOptions.map(v => <option key={v} value={v}>{v}</option>)}
          </select>
        </div>
      </div>

      <div className="f-field">
        <label htmlFor="bs-details">{t.additionalDetails || "Additional details"}</label>
        <textarea id="bs-details" rows={4} placeholder={t.additionalDetailsPlaceholder || "Anything else we should know about this request."} value={form.additionalDetails} onChange={e => update("additionalDetails", e.target.value)} />
      </div>
      ...
      {status === "error" && (
        <p className="f-err req-error">
          {t.errorMsg || "Something went wrong. Email us at contact@kizunaproxy.com and we'll sort it out."}
        </p>
      )}
      ...
      >
        {status === "sending" ? (t.sendingBtn || "SENDING…") : (t.submitBtn || "Request a Business Quote")}
      </button>
    </div>
  );
}
